...
간단한 프로그램은 명령어 방법으로도 충분한데 spec을 다루면 옵션을 저장하고 편하게 사용 가능하다.
...
이렇게 실행하면 build, dist 폴더와 spec파일이 생성되는데 dist폴더 안에 배포물이 준비된다. 
...
```
> 이 옵션은 입력을 안해도 기본적으로 적용된다. 결과는 위에 기본 방법과 같다.

프로그램을 설치하면 "내 컴퓨터 - 프로그램 파일"에 설치된 파일들 처럼 폴더 안에 프로그램 실행에 필요한 구성 요소들이 생성된다.
